...
The user installs jobme into Kokua's environment (`uv add --editable ../jobme`, or `pip install
jobme` once published) and Kokua discovers the toolset through the entry point. `uv add` is the
